package mode_coverage_test_pkg;
import uvm_pkg::*;
...
class mode_coverage_test extends uvm_test;
    ...
    modes_crossing_sequence modes_seq;
    no_SS_sequence no_ss_seq;

    function new(string name = "mode_coverage_test",uvm_component parent = null);
    ...
    super.build_phase(phase);
        ...
        modes_seq                     = modes_crossing_sequence::type_id::create("modes_seq");
       ...
       if (!uvm_config_db#(virtual apb_if)::get(this, "", "APB_IF", apb_object_inst.apb_vif))  // ← apb_if, not apb_object_inst
    ...
    task run_phase (uvm_phase phase);
      ...
      `uvm_info("run_phase","modes_rst_seq stimulus generation started",UVM_LOW)
          modes_rst_seq.start(env.agent.sqr);
      `uvm_info("run_phase","modes_rst_seq stimulus generation ended",UVM_LOW)

      `uvm_info("run_phase","modes_seq stimulus generation started",UVM_LOW)
          modes_seq.start(env.agent.sqr);
      `uvm_info("run_phase","modes_seq stimulus generation ended",UVM_LOW)

      `uvm_info("run_phase","no_ss_seq stimulus generation started",UVM_LOW)
          no_ss_seq.start(env.agent.sqr);
      `uvm_info("run_phase","no_ss_seq stimulus generation ended",UVM_LOW)
